'''
    ...
    Description: Manual quadrotor control
...
def MOTOR_COMMANDS(delta_control,T_cut):
    R=delta_control[0]
    P=delta_control[1]
    T=delta_control[2]
    Y=delta_control[3]
...
if (mode==1 or mode==4):
    ...
    # [10]=system orientation offset x (deg)
    # [11]=system orientation offset x (deg)
    ...
    if mode==4:
        ...
        flt_log.write('t dt phi theta psi phi_dot theta_dot psi_dot m1 m2 m3 m4 theta_target phi_target psi_target delta_roll delta_pitch delta_thr delta_yaw\n')    
    ...
    while exit_flag==0:
        ...
        motor=MOTOR_COMMANDS(delta_control,T_cut)
        ...
        if mode==4:
            ...
            flt_log.write('%.3f %.4f %.2f %.2f %.2f %.2f %.2f %.2f %d %d %d %d %.2f %.2f %.2f %d %d %d %d\n' % (t_elapsed,dt,AHRS_data[0],AHRS_data[1],AHRS_data[2],AHRS_data[4],AHRS_data[5],AHRS_data[3],int(motor[0]),int(motor[1]),int(motor[2]),int(motor[3]),targets[1],targets[0],targets[2],int(delta_control[0]),int(delta_control[1]),int(delta_control[2]),int(delta_control[3])))
